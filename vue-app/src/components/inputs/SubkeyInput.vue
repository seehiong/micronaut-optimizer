<template>
    <textarea v-model="subkey" placeholder="Inner JSON key..." spellcheck="false" @mousedown.stop @mouseup.stop
...
export default {
    ...
    watch: {
        initialData: {
            handler(newValue) {
                if (newValue !== this.subkey) {
                    this.subkey = newValue || "";
                }
            },
            immediate: true
        },
    ...
    methods: {
        onInputChange(event) {
            this.subkey = event.target.value;
            this.$emit("input-change", this.subkey);
        },
    },
};
</script>
